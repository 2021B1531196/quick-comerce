            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    {Array(5).fill('').map((_, i) => (
                            ...
                            <img className="md:w-3.5 w3" key={i} src={i<4 ? assets.star_icon :assets.star_dull_icon } alt="" />
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    {currency} RS-{products.offerPrice}{" "} <span className="text-gray-500/60 md:text-sm text-xs line-through">{currency} RS-{products.price}</span>
                    </p>
                    <div onClick={(e) => {e.stopPropagation();}} className="text-primary">
                        { !cartItems[products._id]? (
                            <button className="flex items-center justify-center gap-1 bg-primary/10 border border-primary/14 md:w-[80px] w-[64px] h-[34px] rounded cursor-pointer " onClick={() => addToCart(products._id)} >
